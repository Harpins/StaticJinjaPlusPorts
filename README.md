...
Все образы используют одинаковый интерфейс:

- `--srcpath <папка>` — путь к вашим шаблонам внутри контейнера (по умолчанию использует `templates_example`)
- Результат генерации будет помещен в `/app/build`
